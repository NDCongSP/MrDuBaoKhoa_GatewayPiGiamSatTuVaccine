...
namespace nhietdokhovacxin
{
    public partial class Form1 : Form
    {
        ...
        static double nhietKenh1 = 0, nhietKenh2 = 0, nhietKenh3 = 0, nhietKenh4;
        ...
        static string conStringLocal = "Server =192.168.100.101; Database = cauhinh ; Port=3306;User ID=root ;Password=100100;charset=utf8";
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                string[] lines = System.IO.File.ReadAllLines("/home/pi/Files/idds18b20.txt");
                label11.Text = TenCB[0] = lines[0].Split('|')[1];
                label12.Text = TenCB[1] = lines[1].Split('|')[1];
                label13.Text = TenCB[2] = lines[2].Split('|')[1];
                label5.Text = TenCB[3] = lines[3].Split('|')[1];
                label14.Text = TenCB[4] = lines[4].Split('|')[1];

                //CHINH CHO NAY
                //string DataParametter = ReadText("./Files/ThoiGianCapNhat.txt").Trim();
                string DataParametter = ReadText("/home/pi/Files/ThoiGianCapNhat.txt").Trim();
                ...
                DataParametter = ReadText("/home/pi/Files/GioiHanNhietDo.txt").Trim();
                ...
                DataParametter = ReadText("/home/pi/Files/Offset.txt").Trim();
                ...
                //if (myPLC.ModbusRTUMaster.KetNoi("COM2", 9600, 8, System.IO.Ports.Parity.None, System.IO.Ports.StopBits.One) == true)
                {
                    ...
                    if (myPLC.ModbusRTUMaster.ReadHoldingRegisters(1, 0, 3, ref docHoldingNhietDo) == true)
                    {
                        demmatketnoi_modbus = 0;

                        nhietKenh1 = nhietKenh1New = Math.Round(Convert.ToDouble(myPLC.GetShortAt(docHoldingNhietDo, 0)) / 10, 2) + offset1;
                        nhietKenh2 = nhietKenh2New = Math.Round(Convert.ToDouble(myPLC.GetShortAt(docHoldingNhietDo, 2)) / 10, 2) + offset2;
                        nhietKenh3 = nhietKenh3New = Math.Round(Convert.ToDouble(myPLC.GetShortAt(docHoldingNhietDo, 4)) / 10, 2) + offset3;

                        Console.WriteLine($"doc nhiet do ban dau thanh cong");
                    }
                    else
                    {
                        demmatketnoi_modbus++;
                    ...
                    if (myPLC.ModbusRTUMaster.ReadHoldingRegisters(3, 0, 1, ref docHoldingNhietDo) == true)
                    {
                        demmatketnoi_modbus = 0;

                        nhietKenh4 = nhietKenh4New = Math.Round(Convert.ToDouble(myPLC.GetShortAt(docHoldingNhietDo, 0)) / 10, 2) + offset4;
                    ...
                    Console.WriteLine($"Nhiệt độ ban đầu: {nhietKenh1.ToString()}|{nhietKenh2.ToString()}|{nhietKenh3.ToString()}|{nhietKenh4.ToString()}" +
                $"//trước Ofset: {nhietKenh1 - offset1}|{nhietKenh2 - offset2}|{nhietKenh3 - offset3}|{nhietKenh4 - offset4}");
                    #endregion
                }
                else
                {
        ...
        private void ghicsdl(Label mylabel1, Label mylabel2, Label mylabel3, Label mylabel4, Label mylabel5)
        {
            try
            {
        ...
        private void DocNhietDo(byte idNhietDo)
        {
            #region doc nhiet do tu 485
            if (idNhietDo == 1)
            {
                if (myPLC.ModbusRTUMaster.ReadHoldingRegisters(1, 0, 3, ref docHoldingNhietDo) == true)
                {
                    ...
                    if (nhietKenh1New <= nhietKenh1 + giaTriVotLo)
                    {
                        nhietKenh1 = nhietKenh1New;
                    }

                    if (nhietKenh2New <= nhietKenh2 + giaTriVotLo)
                    {
                        nhietKenh2 = nhietKenh2New;
                    }

                    if (nhietKenh3New <= nhietKenh3 + giaTriVotLo)
                    {
                        nhietKenh3 = nhietKenh3New;
                    }
                    Console.WriteLine("doc nhiet do thanh cong");
                }
                else
                {
                    demmatketnoi_modbus++;
            ...
            else
            {
                if (myPLC.ModbusRTUMaster.ReadHoldingRegisters(3, 0, 1, ref docHoldingNhietDo) == true)
                {
                    ...
                    nhietKenh4New = Math.Round(Convert.ToDouble(myPLC.GetShortAt(docHoldingNhietDo, 0)) / 10, 2) + offset4;

                    if (nhietKenh4New <= nhietKenh4 + giaTriVotLo)
                    {
            ...
            Console.WriteLine($"Nhiet do: {nhietKenh1.ToString()}|{nhietKenh2.ToString()}|{nhietKenh3.ToString()}|{nhietKenh4.ToString()}" +
                $"//trước Ofset: {nhietKenh1 - offset1}|{nhietKenh2 - offset2}|{nhietKenh3 - offset3}|{nhietKenh4 - offset4}");
            #endregion
        }
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    BaoMatDien(TenCB[4], label4, sdtSms, email);
                }
                else if (dem == 3)
                {
                    DocNhietDo(3);
                    hienthi(nhietKenh3.ToString(), TenCB[2], label3, 2, lowLevel.ToString(), hightLevel.ToString(), sdtSms, email);
                    hienthi(nhietKenh4.ToString(), TenCB[3], label6, 3, lowLevel.ToString(), hightLevel.ToString(), sdtSms, email);
                }
                // cap nhat ia tri len web 
                ttkn_server = mysql.capnhatgiatriweb(label1.Text.Trim(), label2.Text.Trim(), label3.Text.Trim(), label6.Text.Trim(), label4.Text.Trim());
            }
            catch { }
            timer1.Enabled = true;
        }
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                ghicsdl(label1, label2, label3, label6, label4);
            }
            catch { }
            timer2.Enabled = true;
