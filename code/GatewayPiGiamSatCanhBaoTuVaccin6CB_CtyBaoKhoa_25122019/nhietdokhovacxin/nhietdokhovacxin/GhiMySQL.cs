...
namespace nhietdokhovacxin
{
    ...
    {
        public string ChuoiKetnoiMySQL_local = "Server =45.119.212.41 ; Database = ctybaokhoabvculaominhbentre ; Port=3306;User ID=customer_ttp ;Password=ThinhTamPhat!@#456&*(;charset=utf8";
        //public string ChuoiKetnoiMySQL_local = "Server = 192.168.1.36 ; Database = ctybaokhoattytgovap ; Port=3306;User ID=root ;Password=100100;charset=utf8";
        ...
        public string capnhatgiatriweb(string cb1, string cb2, string cb3, string cb4, string cb5, string cb6, string cb7)
        {
            try
            {
                string MysqlCmd = "update hienthiweb set cb1 = '" + cb1 + "',cb2 = '" + cb2 + "',cb3 = '" + cb3 + "',cb4 = '" + cb4 + "',cb5 = '" + cb5 + "',cb6= '" + cb6 + "',cb7 = '" + cb7 + "'";
                MySqlCommand cmd = new MySqlCommand(MysqlCmd, connect_local);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
                cmd = null;
