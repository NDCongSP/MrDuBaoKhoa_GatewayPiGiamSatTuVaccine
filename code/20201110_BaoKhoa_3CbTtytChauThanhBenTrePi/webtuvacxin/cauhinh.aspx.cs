    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["Khoa"] == "Mo")
        {
            if (!IsPostBack)
            {
                ...
                Con = new MySqlConnection("Server =45.119.212.41 ; Database = ctybaokhoattytchauthanhbentre ; Port=3306;User ID=customer_ttp ;Password=ThinhTamPhat!@#456&*(;charset=utf8");
                Con.Open();
                ...
                using (MySqlConnection connection = new MySqlConnection("Server =localhost; Database = cauhinh ; Port=3306;User ID=root ;Password=100100;charset=utf8"))
                {
                    connection.Open();

                    string query = "SELECT * FROM gioihannhietdo";
                    ...
                    {
                        TextBox23.Text = res.Rows[0][2].ToString();
                        TextBox25.Text = res.Rows[0][4].ToString();
                        TextBox26.Text = res.Rows[0][0].ToString();
                        TextBox27.Text = res.Rows[0][1].ToString();
                    }
                }
