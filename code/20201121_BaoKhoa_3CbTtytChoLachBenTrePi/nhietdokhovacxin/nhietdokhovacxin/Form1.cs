...
namespace nhietdokhovacxin
{
    public partial class Form1 : Form
    {
        ...
        static double nhietKenh1New = 0, nhietKenh2New = 0, nhietKenh3New = 0;
        static double giaTriVotLo = 10;
        static double offset1 = 0, offset2 = 0, offset3 = 0;
        static byte idPLC = 1, idNhietDo = 2;
            ...
            try
            {
                ...
                //if (myPLC.ModbusRTUMaster.KetNoi("COM2", 9600, 8, System.IO.Ports.Parity.None, System.IO.Ports.StopBits.One) == true)
                {
                    ...
                    if (myPLC.ModbusRTUMaster.ReadHoldingRegisters(idNhietDo, 0, 3, ref docHoldingNhietDo) == true)
                    {
                        ...
                        nhietKenh1New = Math.Round(Convert.ToDouble(myPLC.GetShortAt(docHoldingNhietDo, 0)) / 10, 2);
                        nhietKenh2New = Math.Round(Convert.ToDouble(myPLC.GetShortAt(docHoldingNhietDo, 2)) / 10, 2);
                        nhietKenh3New = Math.Round(Convert.ToDouble(myPLC.GetShortAt(docHoldingNhietDo, 4)) / 10, 2);

                        if (nhietKenh1New <100 && nhietKenh1New > 0)
                        {
                            nhietKenh1 = nhietKenh1New;
                        }
                        else
                        {
                            nhietKenh1 = nhietKenh1New = 7;
                        }

                        if (nhietKenh2New < 100 && nhietKenh2New > 0)
                        {
                            nhietKenh2 = nhietKenh2New;
                        }
                        else
                        {
                            nhietKenh2 = nhietKenh2New = 7;
                        }

                        if (nhietKenh3New < 100 && nhietKenh3New > 0)
                        {
                            nhietKenh3 = nhietKenh3New;
                        }
                        else
                        {
                            nhietKenh3 = nhietKenh3New = 7;
                        }
        ...
        private void DocNhietDo()
        {
            ...
            if (myPLC.ModbusRTUMaster.ReadHoldingRegisters(idNhietDo, 0, 3, ref docHoldingNhietDo) == true)
            {
                ...
                if (nhietKenh1New < 100 && nhietKenh1New <= nhietKenh1 + giaTriVotLo && nhietKenh1New >= nhietKenh1 - giaTriVotLo)
                {
                    nhietKenh1 = nhietKenh1New;
                }

                if (nhietKenh2New < 100 && nhietKenh2New <= nhietKenh2 + giaTriVotLo && nhietKenh2New >= nhietKenh2 - giaTriVotLo)
                {
                    nhietKenh2 = nhietKenh2New;
                }

                if (nhietKenh3New < 100 && nhietKenh3New <= nhietKenh3 + giaTriVotLo && nhietKenh3New >= nhietKenh3 - giaTriVotLo)
                {
                    nhietKenh3 = nhietKenh3New;
                }

                Console.WriteLine($"doc nhiet do thanh cong {nhietKenh1.ToString()}|{nhietKenh2.ToString()}|{nhietKenh3.ToString()}" +
