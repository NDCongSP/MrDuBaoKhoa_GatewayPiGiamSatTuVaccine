...
namespace nhietdokhovacxin
{
    public class GhiMySQL
    {
        ...
        public string capnhatgiatriweb(string cb1, string cb2, string cb3, string cb4, string cb5, string cb7)
        {
            try
            {
                string MysqlCmd = "update hienthiweb set cb1 = '" + cb1 + "',cb2 = '" + cb2 + "',cb3 = '" + cb3 + "',cb4 = '" + cb4 + "',cb5 = '" + cb5 + "',cb7 = '" + cb7 + "'";
                MySqlCommand cmd = new MySqlCommand(MysqlCmd, connect_local);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
                cmd = null;
