...
namespace nhietdokhovacxin
{
    ...
    {
        public string ChuoiKetnoiMySQL_local = "Server = 103.48.195.37 ; Database = ctybaokhoa ; Port=3306;User ID=plcpi_giamsat ;Password=12345;charset=utf8";
        ...
        public string capnhatgiatriweb(string cb1, string cb2, string cb3, string cb4, string cb5, string cb6, string cb7, string cb8, string cb9)
        {
            try
            {
                string MysqlCmd = "update hienthiweb set cb1 = '" + cb1 + "',cb2 = '" + cb2 + "',cb3 = '" + cb3 + "',cb4 = '" + cb4 + "',cb5 = '" + cb5 + "',cb6= '" + cb6 + "',cb7 = '" + cb7 + "',cb8 = '" + cb8 + "',cb9 = '" + cb9 + "'";
                MySqlCommand cmd = new MySqlCommand(MysqlCmd, connect_local);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
                cmd = null;
